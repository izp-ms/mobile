...
class FollowedByPage extends StatefulWidget {
  const FollowedByPage({Key? key}) : super(key: key);

  @override
  State<FollowedByPage> createState() => _FollowedByPageState();
}

class _FollowedByPageState extends State<FollowedByPage>
    ...
    listScrollController.addListener(() {
      if (listScrollController.position.atEdge) {
        if (listScrollController.position.pixels != 0) {
          BlocProvider.of<FollowedByCubit>(context).getFollowedBy(
              search, orderBy);
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        BlocConsumer<FollowedByCubit, FollowedByState>(
